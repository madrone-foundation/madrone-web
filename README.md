...
- **Leadership page TODOs**: LinkedIn URLs for Nick Waugh, Blair Tavenner, and Michael Charlebois are placeholder `#` links (marked with HTML comments); board roster section is scaffolded — replace the placeholder row with one `dev-item` per director (name, one-sentence bio, affiliation). A headshot for Michael can be dropped in as `images/team/michael-charlebois.jpg`.
